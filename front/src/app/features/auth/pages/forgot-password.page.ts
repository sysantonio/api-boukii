          ...
          <form [formGroup]="form" (ngSubmit)="submit()" novalidate>
            ...
            <div class="card-actions">
              ...
              <nav class="auth-links">
                <a routerLink="/auth/login">{{ t('auth.forgotPassword.links.login') }}</a>
              </nav>
            </div>
          </form>
        }
        ...
        @if (emailSent()) {
          <div class="success-state">
            ...
            <div class="success-content">
              <h3 class="success-title">{{ t('auth.forgotPassword.successTitle') }}</h3>
              <p class="success-message">{{ statusMessage() }}</p>
              <p class="help-text">{{ t('auth.forgotPassword.checkSpam') }}</p>
            </div>
...
export class ForgotPasswordPage implements OnInit {
  ...
  submit(): void {
    ...
    const sentMsg = this.translationService.get('auth.forgotPassword.emailSent');
    const processingMsg = this.translationService.get('auth.forgotPassword.processing');
  ...
  getFieldError(field: string): string {
    ...
    if (c.errors['required']) return this.translationService.get('auth.errors.requiredEmail');
    if (c.errors['email']) return this.translationService.get('auth.errors.invalidEmail');
    return this.translationService.get('auth.errors.fieldInvalid');
  }
